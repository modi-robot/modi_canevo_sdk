 ...
 * @brief CSV速度模式测试 - 外部实时线程调用 RtStepOnce()
 */
...
constexpr int kTotalCycles =
    (3 * 1000000 / kPeriodUs) + (1 * 1000000 / kPeriodUs) +
    (3 * 1000000 / kPeriodUs) + (200000 / kPeriodUs);
...
float TargetVelocity(int cycle) {
  const int forward_cycles = 3 * 1000000 / kPeriodUs;
  const int pause_cycles = 1 * 1000000 / kPeriodUs;
  const int reverse_cycles = 3 * 1000000 / kPeriodUs;

  if (cycle < forward_cycles) return 1.0f;
  if (cycle < forward_cycles + pause_cycles) return 0.0f;
  if (cycle < forward_cycles + pause_cycles + reverse_cycles) return -1.0f;
  return 0.0f;
}
...
void* CsvRtLoop(void*) {
  ...
  while (true) {
    ...
    const int set_ret = g_joint->RtSetCsvTargetVelocity(TargetVelocity(cycle));
...
int main() {
  ...
  pthread_t rt_thread{};
  const int create_ret = CreateRealtimeThread(&rt_thread, CsvRtLoop);
  if (create_ret != 0) {
