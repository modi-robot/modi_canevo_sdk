 ...
 * @brief CST 电流模式测试 - 外部实时线程调用 RtStepOnce()
 *
...
constexpr float kTargetCurrentA = 2.0f;
...
void* RtLoop(void*) {
  ...
  while (g_run_state.load(std::memory_order_acquire) != RunState::kRtExit) {
    ...
    const bool in_cst = g_joint && current_mode == CanEvoMode::kCst &&
...
int main() {
  std::cout << "========================================" << std::endl;
  std::cout << "CST 测试程序 - 持续下发 0 A 目标电流" << std::endl;
  std::cout << "========================================" << std::endl;
  qiuniu_init();
  ...
  if (!WaitControlMode(joint, CanEvoMode::kCst, "等待 CST 模式切换")) {
    pthread_attr_destroy(&attr);
    joint.NrtDestroy();
    bus.Close();
    return -1;
  ...
  if (create_ret == 0) {
    __RT(pthread_setname_np(rt_thread, "canevo_cst_rt"));
  }
  pthread_attr_destroy(&attr);
  if (create_ret != 0) {
  ...
  std::cout << "✓ 关节已切换到 CST 模式" << std::endl;
