 ...
 * @brief 多关节 CSV 速度模式测试 - 外部实时线程调用 RtStepOnce()
 *
...
void* RtLoop(void*) {
  ...
  std::vector<float> vel_rad_s(g_joints->size(), kCsvSpeedRadS);
  for (size_t i = 0; i < g_joints->size(); ++i) {
    const float pos = (*g_joints)[i].NrtGetActualPosition();
    if (pos >= kCsvMaxPosRad) {
      vel_rad_s[i] = -kCsvSpeedRadS;
    } else if (pos <= kCsvMinPosRad) {
      vel_rad_s[i] = kCsvSpeedRadS;
    }
  }
  ...
  while (g_run_state.load(std::memory_order_acquire) != RunState::kRtExit) {
    ...
    for (size_t i = 0; i < g_joints->size(); ++i) {
      ...
      const bool in_csv = current_mode == CanEvoMode::kCsv &&
      ...
      const int set_ret = joint.RtSetCsvTargetVelocity(vel_rad_s[i]);
...
int main() {
  std::cout << "========================================" << std::endl;
  std::cout << "CSV 测试程序 - 多关节在 ±90° 内以 10°/s 往复运动" << std::endl;
  std::cout << "========================================" << std::endl;
  qiuniu_init();
  ...
  for (size_t i = 0; i < joints.size(); ++i) {
    ...
    if (!WaitControlMode(joints[i], CanEvoMode::kCsv, "等待 CSV 模式切换")) {
      PrintJointDiag(joints[i], joint_ids[i]);
      pthread_attr_destroy(&attr);
      for (auto& joint : joints) {
        joint.NrtDestroy();
  ...
  if (create_ret == 0) {
    __RT(pthread_setname_np(rt_thread, "canevo_multi_csv"));
  }
  pthread_attr_destroy(&attr);
  if (create_ret != 0) {
  ...
  std::cout << "✓ 全部关节已切换到 CSV 模式" << std::endl;
